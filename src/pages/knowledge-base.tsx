...
export default function KnowledgeBasePage() {
  ...
  const processLocalFile = (file: File) => {
    ...
    else if (['pdf', 'doc', 'docx'].includes(ext)) detectedType = 'document';
    else if (['js', 'ts', 'py', 'html', 'css', 'json'].includes(ext)) detectedType = 'code';
    ...
    reader.onload = (e) => {
      ...
      // Clean non-printable binary artifacts if PDF or raw document
  ...
  const handleUploadDocument = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !content) {
      alert('Document name and text content are required');
      return;
    }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white mt-2">
              Knowledge Base & Local Storage Ingestion
            </h1>
            <p className="text-sm text-slate-400 mt-1 max-w-3xl">
              Ingest enterprise documents directly from your <strong>Local Computer Device / Storage</strong> or paste raw text. Automatically chunk, generate vector embeddings, and search with tenant isolation.
            </p>
          </div>
        </div>
      </div>
            ...
            <form onSubmit={handleUploadDocument} className="space-y-4 text-xs">
                ...
                <div>
                  ...
                  >
                    <input
                      ...
                      accept=".txt,.md,.markdown,.json,.csv,.pdf,.doc,.docx,.html,.js,.ts,.py,.log,.xml,.yaml,.yml"
                      className="hidden"
                    />
                      ...
                      <div className="space-y-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400 font-mono mt-1">
                            Supports .md, .txt, .pdf, .docx, .json, .csv, code files
                          </p>
                        </div>
                      </div>
                    )}
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Acme_SLA_Policy.md"
                  value={name}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-400 mb-1 font-mono">File Type:</label>
                  <select
                    value={fileType}
            ...
            {searchResults.length > 0 && (
              <div className="space-y-3 pt-2">
                ...
                {searchResults.map((res, i) => (
                  <div key={i} className="p-3.5 rounded-xl bg-surface-200 border border-cyan-500/30 space-y-1">
                    ...
                    <p className="text-xs text-slate-300 font-mono bg-black/20 p-2 rounded">
                      {res.content}
                    </p>
                  </div>
                ))}
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              ...
              <div className="glass-panel p-8 rounded-2xl text-center text-slate-400 text-sm">
                No documents ingested in this organization yet. Use the local file uploader on the left to add your first document.
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-2 line-clamp-2 font-mono bg-black/20 p-2 rounded">
                          {doc.content}
                        </p>
                      </div>
                    ...
                    <div className="pt-3 border-t border-white/5 flex items-center justify-between text-xs text-slate-400 font-mono">
                      <span className="flex items-center gap-1 text-cyan-300 font-semibold">
                        <Layers className="h-3.5 w-3.5" /> {doc.chunks?.length || 1} Chunks Vectorized
                      </span>
